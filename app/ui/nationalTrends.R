df_pfa2 <- df_pfa[df_pfa$pfaName %in% "Avon and Somerset",]
df_pfa2 <- subset(df_pfa2, select = c(2, 11))
df_pfa2 <- aggregate(. ~ financialYear, df_pfa2, function(x) sum(as.numeric(x)))
      ...
      conditionalPanel(
        ...
        div(style="height: 100vh",
            ...
            pickerInput(
              ...
              column(7,            
                     br(),
                     HTML("<H2> Avon and Somerset 2011/12-2021/2022 </H2>"),
                                #div(wellPanel(h1("panel")), style="width=100%", height="10%"),
                                div(
                                  hchart(
                                    df_pfa2,
                                    "line",
                                    hcaes(x = financialYear, y = numberOfSearches),

                            
                                  )
                                ),
                     "There were X stop-searches, with legislation Y")
            )
        )
      )
    )
